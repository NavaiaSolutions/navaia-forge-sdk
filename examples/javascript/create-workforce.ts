...
console.log(`Workforce: ${workforce.id}`);
...
const writer = await nf.agents.create({
  workforce_id: workforce.id,
  name: "Writer",
  role: "writing",
  instructions:
...
console.log(
  `Agents created: ${researcher.name}, ${writer.name}, ${reviewer.name}`,
);

// ── 3. Connect agents with edges ───────────────────────────
// Edges define the flow: Researcher -> Writer -> Reviewer

const workforceFull = await nf.workforces.getFull(workforce.id);
console.log(`Workforce has ${workforceFull.agents.length} agents`);

// NOTE: Edge creation is typically done via the workforce update API
// or through the visual canvas in the NavaiaForge UI.

await nf.workforces.update(workforce.id, {
  config_json: {
    edges: [
      {
